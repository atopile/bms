...
//     aTxBuf[0] = (uint8_t)((uint16_t)(nCommand & (uint16_t)0xFF00U) >> 8U);
//     aTxBuf[1] = (uint8_t)(nCommand & (uint16_t)0x00FFU);
//     /* Calculate the 15-bit PEC for the command bytes */
//     nCmdPec = (uint16_t)calculatePEC15(&aTxBuf[0], 2u);
//     /* Append the command PEC to the transmit buffer */
//     aTxBuf[2] = (uint8_t)(nCmdPec >> 8U);
//     aTxBuf[3] = (uint8_t)(nCmdPec);
...
//     // for (nByteIndex = 0u; nByteIndex < ADBMS6948_REG_GRP_LEN; nByteIndex++)
//     // {
//     //     aTxBuf[le] = *(pDevCfgArray + nByteIndex);
//     // }
//     aTxBuf[4] = pTxBuf[0];
//     /* Calculate the 10-bit PEC for the transmit data bytes for all the devices on the chain */
//     nCfgPec = (uint16_t)calculatePEC10(pDevCfgArray, false, ADBMS6948_REG_GRP_LEN);
//     /* Append the data PEC to the transmit buffer */
//     aTxBuf[5]  = (uint8_t)((nCfgPec & (uint16_t)0x0300u) >> 8u);
//     aTxBuf[6] = (uint8_t)(nCfgPec & (uint16_t)0x00FFu);
//     // }
//     Serial.print("aTxBuf: ");
//     for (int i = 0; i < 6; i++) {
//         Serial.print(aTxBuf[i], HEX);
...
void loop() {
    ...
    uint16_t nCommand;
    uint16_t nCmdPec;
    uint8_t aCmd[12]; // Adjusted size to accommodate all elements
    nCommand = RDCVA;
    aCmd[0] = (uint8_t)((nCommand & 0xFF00u) >> ADBMS6948_SHIFT_BY_8);
    aCmd[1] = (uint8_t)((nCommand & 0x00FFu));
    /* Calculate the 15-bit PEC for the command bytes */
    ...
    aCmd[4] = 0x00; // Uncomment if necessary
    aCmd[5] = 0x00; // Uncomment if necessary
    aCmd[6] = 0x00; // Uncomment if necessary
    aCmd[7] = 0x00; // Uncomment if necessary
    aCmd[8] = 0x00; // Uncomment if necessary
    aCmd[9] = 0x00; // Uncomment if necessary
    aCmd[10] = 0x00; // Uncomment if necessary
    aCmd[11] = 0x00; // Uncomment if necessary

    // uint8_t aData[1] = {0x87};
    // aCmd[4] = aData[0];
    // // aCmd[5] = 0x00; // Uncomment if necessary

    // uint16_t dataPEC = calculatePEC10(aData, 1, false);
    // aCmd[5] = (uint8_t)(dataPEC >> ADBMS6948_SHIFT_BY_8);
    // aCmd[6] = (uint8_t)(dataPEC);

    Serial.print("aTxBuf: ");
    for (int i = 0; i < 12; i++) { // Loop through the correct buffer size
        Serial.print(aCmd[i], HEX);
        Serial.print(" ");
    }
    Serial.println();
    ...
    float voltage = convertToVoltage(aCmd[5], aCmd[4]);
